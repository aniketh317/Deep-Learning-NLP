...
class ENBertReverseDict(nn.Module):
    ...
    def forward(self, input):
        ...
        #  batch_size x max_len x vocab_size  --This vocab size is for the subtokens(ie. number of subtokens)
        bpe_reps = self.bert_model(input_ids=input, token_type_ids=type_ids,
                                                        attention_mask=attention_mask)[0]

        # bsz x max_word_len x word_vocab_size --word_vocab_size is the number of words in target dictionary
        word2bpes = self.word2bpes.repeat(bpe_reps.size(0), 1, 1)
        word_scores = bpe_reps.gather(dim=-1, index=word2bpes)   # bsz x max_word_len x word_vocab_size

        word_scores = word_scores.sum(dim=1)
        if self.training:
            word_scores = word_scores[:, :self.number_word_in_train] #(bsz x word_vocab_size)
