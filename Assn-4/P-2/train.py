...
def model_train(model,train_data, criterion, optimiser, collate_fn, embed=None, hidden=None, device = torch.device('cpu'), epoch_l=300, 
    ...
    for batch_idx, (Xb,lab,lend,lens) in enumerate(vloader):
        pre = model.predict(X=Xb.to(device), lend=lend.to(device, dtype=torch.int), lens=lens.to(device,dtype=torch.int)) #(N, )
    ...
    for batch_idx, (Xb,lab,lend,lens) in enumerate(tloader):
        b_loss = criterion(model(X=Xb.to(device), lend=lend.to(device, dtype=torch.int),lens=lens.to(device, dtype=torch.int)), lab.to(device))
