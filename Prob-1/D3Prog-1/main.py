...
ft = fasttext.load_model('/raid/home/anikethv/.local/bin/cc.en.300.bin')
...
N = 2 #Number of processes(Number of GPU devices being used)
...
for grams in train_neg_list:
    num_tokens = 0
    sg = torch.zeros(dim, dtype=torch.float)
    sf = torch.zeros(dim, dtype=torch.float)
    for word in grams:
        if word in embed_dict:
            sg += embed_dict[word]
            sf += ft.get_word_vector(word)
            num_tokens += 1
    if num_tokens != 0:
        sg = sg/num_tokens
        sf = sf/num_tokens
...
def worker(rank):
    ...
    Path = '/raid/home/anikethv/others/DL-NLP/Prob-1/D3Prog-1/Models/'
