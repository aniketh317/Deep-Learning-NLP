...
def model_train(model,train_data, criterion, optimiser, collate_fn, embeddings, epoch_l=300, verbose=False, patience=40):
    ...
    while epoch <= epoch_l and not stop:
        ...
        tloader = DataLoader(tdata, batch_size=256, shuffle=True, num_workers=0, collate_fn=collate_fn)
        ...
        vacc = 0.0
        vloader = DataLoader(vdata, batch_size=lengths[1], shuffle=False, collate_fn=collate_fn)
        ...
        if not Macc or vacc>Macc:
            Macc = vacc
            for name, param in model.named_parameters():
                save_weights[name] = (param.data.clone().detach().requires_grad_(True), param.grad)
            best_embeddings = embeddings.clone().detach().requires_grad_(True)
    ...
    with torch.no_grad():
        ...
        embeddings.copy_(best_embeddings)
